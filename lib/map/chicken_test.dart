...
class Dungeon {
  ...
  void _generateChickens() {
    for (int i = 0; i < roomList.length; i++) {
      if (rand.nextDouble() < 0.2) {
        final int chickenPoint = rand.nextInt(roomList[i].length);
        final int checkenX = roomList[i][chickenPoint].x.toInt();
        final int checkenY = roomList[i][chickenPoint].y.toInt();
        map[checkenX][checkenY] = 7;
        chickenList.add(Point(checkenX, checkenY));
      }
    }
  }
  ...
  void _generateRooms() {
    ...
    roomList = List.generate(roomCount, (_) => []);
    ...
    for (int i = 0; i < roomCount; i++) {
      ...
      for (int dx = 0; dx < roomWidth; dx++) {
        for (int dy = 0; dy < roomHeight; dy++) {
          map[x + dx][y + dy] = 1;
          roomList[i].add(Point(x + dx, y + dy));
        }
      }
